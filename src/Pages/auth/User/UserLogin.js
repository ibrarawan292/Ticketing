...
function UserLogin() {
  ...
  const { DarkMode, setRefreshUserLogin, RefreshUserLogin } =
    ...
    fetch(ApiURL + "/user/login", requestOptions)
      ...
      .then((result) => {
        ...
        if (result.status === 200) {
          localStorage.setItem("AccessTokenUser", result.access_token);
          localStorage.setItem("RefreshTokenUser", result.refresh_token);
          localStorage.setItem("UserIsLogin", "true");
          setRefreshUserLogin(RefreshUserLogin === true ? false : true);
          setBtnDisabled(false);
          navigate("/user-dashboard/main-dashboard-user");
  ...
  return (
    <div className="w-full h-screen flex flex-col md:flex-row">
      <Helmet>
        <title>Ticket System | User | Login</title>
      </Helmet>
      <ToastContainer />

      {/* left hand side  */}
      <div className="w-full md:w-[40%] md:h-full bg-red-800">
        <div className="px-8 sm:px-20 md:px-10 lg:px-20 text-white py-16 h-[20%] text-3xl font-serif font-extrabold">
          Ticket System
        </div>
      ...
      <div className="w-full md:w-[60%] h-full relative flex items-center justify-center">
        ...
        <form className="w-10/12 sm:w-2/3 mt-14 md:mt-0 text-center pb-20">
          <div
            style={
              DarkMode === true
                ? { color: "var(--txtColor2)" }
